...
import {
  ...
  getTokenDetails,
  mappingChainIdToAaveV3ProviderAddress,
  mappingChainIdToAaveV3UIProviderAddress,
  mappingChainIdToChainName,
  mappingChainIdToProvider,
} from "../utils/utils";
import { aaveV3UIProviderABI } from "../utils/abi";
import * as fs from "fs";

export const newAaveV3Allowlist = async (): Promise<{
  ...
  try {
    const currentCollaterals: CollateralTokenType[] = collaterals.filter(
      (vault) => vault.protocol === Protocol.AaveV3
    ...
    const chains = Object.keys(mappingChainIdToAaveV3UIProviderAddress);
    ...
    for (const chainId of chains) {
      const uiProviderContract = new ethers.Contract(
        mappingChainIdToAaveV3UIProviderAddress[chainId],
        aaveV3UIProviderABI,
        mappingChainIdToProvider[chainId]
      );

      const reservesDatas = await uiProviderContract.getReservesData(
        mappingChainIdToAaveV3ProviderAddress[chainId]
      );
      ...
      reservesDatas[0].forEach((reserveData: any) => {
        if (reserveData.isActive === false || reserveData.isFrozen === true || reserveData.isPaused === true) {
          return;
        }
    ...
    console.error("Error fetching Aave3 allowlist:", error);
    return {
      newCollaterals: [],
      newVariableDebts: [],
    };
