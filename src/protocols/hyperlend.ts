...
import {
  ...
  mappingChainIdToHyperLendProviderAddress,
  mappingChainIdToHyperLendUIProviderAddress,
  mappingChainIdToProvider,
} from "../utils/utils";
import * as fs from "fs";
import { hyperlendUIProviderABI } from '../utils/abi';

export const newHyperlendAllowlist = async (): Promise<{
  ...
  try {
    const currentCollaterals: CollateralTokenType[] = collaterals.filter(
      (vault) => vault.protocol === Protocol.HyperLend
    ...
    const chains = Object.keys(mappingChainIdToHyperLendUIProviderAddress);
    ...
    for (const chainId of chains) {
      const uiProviderContract = new ethers.Contract(
        mappingChainIdToHyperLendUIProviderAddress[chainId],
        hyperlendUIProviderABI,
        mappingChainIdToProvider[chainId]
      );

      const reservesDatas = await uiProviderContract.getReservesData(
        mappingChainIdToHyperLendProviderAddress[chainId]
      );
    ...
    console.error("Error fetching HyperLend allowlist:", error);
    return {
      newCollaterals: [],
      newVariableDebts: [],
    };
