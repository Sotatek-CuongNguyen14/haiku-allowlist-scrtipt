...
import {
  ...
  mappingChainIdToYeiProviderAddress,
  mappingChainIdToYeiUIProviderAddress,
} from "../utils/utils";
import * as fs from "fs";
import { yeiUIProviderABI } from '../utils/abi';

export const newYeiAllowlist = async (): Promise<{
  ...
  try {
    const currentCollaterals: CollateralTokenType[] = collaterals.filter(
      (vault) => vault.protocol === Protocol.Yei
    ...
    const chains = Object.keys(mappingChainIdToYeiUIProviderAddress);
    ...
    console.error("Error fetching Yei allowlist:", error);
    return {
      newCollaterals: [],
      newVariableDebts: [],
    };
